...
import type { LedgerBlock } from "@/lib/value-ledger";
...
const styles = StyleSheet.create({
  ...
  // ─── Value ledger block (2026-08-06). Realized is the visually loudest of
  // the three even though it is the smallest figure — that inversion is the
  // credibility play, not a styling accident. Modeled is the quietest.
  // ⚠️ HEIGHT IS A FEATURE CONSTRAINT HERE, NOT A STYLE PREFERENCE. This is the
  // TWO-page document a champion forwards. The first render of this block ran
  // the readout to THREE pages, which quietly broke the premise of the whole
  // artifact. Every value below is sized to fit the block in the slack that
  // exists — if you grow it, rasterize and count pages before shipping.
  ledgerBox: {
    borderTopWidth: 1,
  ...
  ledgerTitle: { fontSize: 10.5, color: COLORS.accent, marginBottom: 2 },
  ledgerLead: { fontSize: 8, color: COLORS.faint, lineHeight: 1.35, marginBottom: 5 },
  ledgerRow: {
    flexDirection: "row",
  ...
  ledgerLabel: { fontSize: 8.5, color: COLORS.sub, flex: 1, paddingRight: 10 },
  ledgerLabelQuiet: { fontSize: 8, color: COLORS.faint, flex: 1, paddingRight: 10 },
  ledgerRealizedNum: { fontSize: 16, color: COLORS.ink },
  ledgerSubNum: { fontSize: 12, color: COLORS.sub },
  ledgerModeledNum: { fontSize: 9.5, color: COLORS.faint },
  ledgerNote: { fontSize: 7, color: COLORS.faint, lineHeight: 1.35, marginTop: 4 },
  footer: {
    position: "absolute",
    bottom: 26,
    left: 48,
...
const COPY = {
  ...
  footer:
    "Generated by Viridescent from your organization's own records. Nothing on this page is estimated on your behalf — every rate behind a dollar figure is one your organization entered.",

  // ─── VALUE LEDGER BLOCK (2026-08-06) ──────────────────────────────────────
  // ⚠️ DRAFT CUSTOMER-FACING COPY — PENDING BRIAN.
  //
  // Sits BELOW the years-of-judgment anchor, never above. Years of judgment
  // remains the headline of this document; this is the arithmetic underneath it.
  // Renders ONLY when the org has entered rates — see toReadoutBlock().
  ledgerTitle: "What that cost to acquire",
  ledgerLead:
    "Priced entirely with rates your organization entered. An acquisition cost, never a saving.",
  ledgerRealized: "Realized — measurably happened",
  ledgerSubstitution: "Substitution — work you'd otherwise have bought",
  ledgerModeled: "Modeled — exposure no longer carried (a range, never one number)",
  ledgerExcluded: (n: number) =>
    `${n} captured ${
      n === 1 ? "framework was" : "frameworks were"
    } too thin to value confidently and ${n === 1 ? "is" : "are"} excluded above. `,
};
...
export function ReadoutDocument({
  readout,
  ledger,
}: {
  readout: Readout;
  /** null → this document renders exactly as it did in v2.59. */
  ledger?: LedgerBlock | null;
}) {
  const r = readout;
  const c = r.captured;
  const l = ledger ?? null;
        ...
        <View style={styles.section}>
          ...
          {c.years_of_judgment !== null && c.contributors.length > 0 && (
            <View style={styles.anchorBox}>
              {/* ⚠️ NO "≥" HERE (2026-08-06, found by rasterizing). react-pdf's
                  default Helvetica has no glyph for it, so the product's
                  highest-stakes number rendered as a mangled overlap on top of
                  the first digit — on the one page a budget holder reads.
                  Nothing is lost: COPY.anchor() below already opens with "At
                  least this much experience" whenever years_is_partial. The web
                  page keeps the ≥, where the browser font has it. */}
              <Text style={styles.anchorNum}>{c.years_of_judgment} years</Text>
          ...
          {/* ─── THE LEDGER BLOCK — BELOW the anchor, always. ───
              Renders only when the org entered rates. Realized first and
              largest even though it is the smallest figure: a skeptic reading
              top-down must meet the honest number first. Modeled is a RANGE,
              never a point. */}
          {l && (
            <View style={styles.ledgerBox}>
              <Text style={styles.ledgerTitle}>{COPY.ledgerTitle}</Text>
              <Text style={styles.ledgerLead}>{COPY.ledgerLead}</Text>

              {/* ⭐ A TIER WITH NO FIGURE DOES NOT APPEAR. On screen an
                  unpriced line is useful — it tells the CFO which rate to go
                  enter. On the document a champion forwards it is a blank next
                  to a heading, which is the half-populated state this build
                  promises never leaves the building. toReadoutBlock() already
                  refuses to return a block with no figures at all. */}
              {l.realized !== null && (
                <View style={styles.ledgerRow}>
                  <Text style={styles.ledgerLabel}>{COPY.ledgerRealized}</Text>
                  <Text style={styles.ledgerRealizedNum}>{money(l.realized)}</Text>
                </View>
              )}
              {l.substitution !== null && (
                <View style={styles.ledgerRow}>
                  <Text style={styles.ledgerLabel}>{COPY.ledgerSubstitution}</Text>
                  <Text style={styles.ledgerSubNum}>{money(l.substitution)}</Text>
                </View>
              )}
              {l.modeled_low !== null && l.modeled_high !== null && (
                <View style={styles.ledgerRow}>
                  <Text style={styles.ledgerLabelQuiet}>{COPY.ledgerModeled}</Text>
                  <Text style={styles.ledgerModeledNum}>
                    {`${compact(l.modeled_low)} – ${compact(l.modeled_high)}`}
                  </Text>
                </View>
              )}

              {/* ⭐ THE DATED ASSUMPTIONS FOOTNOTE, and the excluded count with
                  it. Whoever receives this file can reproduce every figure on it
                  from the numbers printed here — a dollar figure a reader cannot
                  reproduce is a dollar figure a reader stops believing.
                  ONE Text, not two: this document is two pages and this block
                  has to earn every point of height it takes. */}
              <Text style={styles.ledgerNote}>
                {(l.excluded_unscoreable > 0 ? COPY.ledgerExcluded(l.excluded_unscoreable) : "") +
                  l.assumptions_line}
              </Text>
            </View>
          )}
