...
### August 5, 2026 - Training Studio generate: raise maxDuration to 300 and surface timeouts at the button
**Context:** "Build the training" was 504ing on Vercel - generation runs longer than the route's cap. The route carried `maxDuration = 60` plus a July 26 comment declaring 60 "the ceiling on this plan," and on a 504 the client's error rendered at the top of the page (off-screen from the button), reading as a button stuck on "Writing the floor version..." forever.
**Decision:** Set `export const maxDuration = 300` on `/api/training-studio/[id]/generate`, and make the client handle failure where the leader is looking: parse the response defensively (a gateway 504 returns HTML, not JSON - `res.json()` was throwing into the generic catch), show "Generation timed out - try again" inline next to the generate button, reset the button, clear the error on the next click.
**Options considered:** Keep 60 and split generation further (more calls, more moving parts); move generation to Inngest (right long-term shape, too heavy for this fix).
**Reasoning:** The three-call floor/supervisor/exec architecture stays - it is the fidelity story, not just a timeout workaround. 300 buys headroom for slow model calls without restructuring. The July "60 is the ceiling" note is treated as stale (Vercel's fluid compute allows 300 on lower plans now); if the deploy rejects it, the Vercel build log will say so explicitly.
**Result:** Local build clean. A timeout now reads as a visible, retryable error instead of a silent hang - and retry resumes from whatever altitude was already written.

### August 5, 2026 - Repeatable demo reset: npm run reset-demo restores AWIP to post-seed state
**Context:** Live-demo prep needed a one-command reset - onboarding had been walked through on demo seats (16 onboarding_progress rows) and Training Studio testing left residue (3 live-created requests with their detection/prescription chains and pending "still watching" attempt rows, plus 2 live-generated trainings including today's written-framework floor version from /training-studio/[id]/generate).
**Decision:** Built scripts/reset-demo.mjs + npm script "reset-demo". Dry-run by default, --confirm to execute, idempotent. Scope discriminators, not date heuristics: org resolved by is_demo=true + name containing "AWIP" (exactly one match required); test training_requests are any with created_via != 'seed-awip-training-demo' (live default is 'training-studio-v1'); test trainings are generated_by='training-studio-format-v1' (the live generate route's marker). Deletes children before parents (outcomes -> requests -> trainings -> fidelity -> prescriptions -> detections), then repairs the four seeded storylines to post-seed state (status 'generated', attempt 1, current_training_id repointed at the seeded version, attempt-1 outcome back to pending) and deletes onboarding_progress for @awip-demo.example seats only, so every demo role auto-routes into fresh onboarding on next login.
**Guardrails:** bdhicks83@gmail.com and bdhicks83+test1@gmail.com asserted untouchable (run aborts if either appears in any deletion set); onboarding deletion is domain-filtered to @awip-demo.example; unknown generated_by markers are reported, never deleted; seed markers (seed-awip-demo / seed-awip-training-demo / seed-p9-effective-training) are never deleted.
**Result:** Ran live Aug 5: 16 onboarding rows, 3 test requests + 3 attempt rows + 2 generated trainings + 3 prescriptions/detections deleted; all 4 seeded storylines verified intact at status 'generated'. Second --confirm run found zero residue - idempotency confirmed. Verify phase is built in: demo-seat onboarding rows = 0, test requests = 0, live-generated trainings = 0, seeded storylines linked. Two fixes en route: retry on transient listUsers ECONNRESET, and onboarding_progress has a composite PK (user_id, track) with no id column, so the delete-counter selects user_id there.

### August 5, 2026 - Generate 502 root-caused: the SDK's 60s timeout was the wall, not Vercel's - per-call 240s override on the 300s route
**Context:** Hours after the maxDuration=300 fix shipped, "Build the training" failed again - this time as the route's own 502 "The training generator flaked - nothing was stored," not a 504. The demo reset script had just run, so deleted rows were the prime suspect. Vercel runtime logs gave the real error: `generateFormatTraining: failed after 1 attempts: Error: Request timed out.` from inside the Anthropic SDK with `status: undefined` - the SDK's client-side abort, not an API or gateway error.
**Root cause:** src/lib/claude.ts constructs ONE shared Anthropic client with `timeout: 60_000`, deliberately sized (July) so a stalled connection fails inside the then-universal 60s function wall. Raising the route's maxDuration to 300 moved Vercel's wall but the SDK still killed every call at 60s - the exact calls the 300s wall was raised FOR. The timeout has two layers, and only one was raised. Verification proved it: the floor generation ran 109.4s.
**Reset script exonerated:** the failing request passed every DB gate (format chosen, prescription linked, grounding found) before dying at the model call, and the dry-run showed all 4 seeded storylines "already clean," invariants intact. The one 404 in the logs was a stale browser tab on a test request the reset had correctly deleted. No reset-demo.mjs changes needed.
**Decision:** Per-request override - `messages.create(body, { timeout: 240_000 })` - on exactly the two calls that run from the 300s route (generateFormatTraining, reframeTrainingAltitude). The shared client keeps its 60s default for every 60s route, and its constructor now carries a warning: any call from a longer-wall route MUST override per request. 240s leaves headroom inside the 300s wall for grounding + the insert.
**Verified end-to-end on production (61502dd):** script-authed as chuck.milner (manager seat), drove the real API on the previously-failing request - format chosen, floor written in 109.4s (would have died at 60s before the fix), supervisor 19.2s, exec 8.9s, request flipped to 'generated', training stored and visible. Then reset dry-run caught exactly the test residue (request + v1 training + attempt row + prescription chain), --confirm restored post-seed state, all verify checks green, demo shoot-ready.
**Lesson:** "timeout" is layered - the platform wall (Vercel maxDuration -> 504) and the HTTP client's own abort (SDK -> "Request timed out", status undefined). Raising one without auditing the other just moves where the same slow call dies, and the error signature tells you which layer fired. When a shared client encodes an infrastructure assumption (60s wall), every route that breaks the assumption needs a per-call override - and the client's constructor is where the warning belongs.
