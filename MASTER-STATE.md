...
⭐⭐⭐ **P-0.5 SHIPPED (July 23) — code deployed to production, migration applied on LIVE Supabase, build verified serving.** Methodology Router (5 methods, offer+suggest), Entity Map (field #8, names kept internally), 20-min session cap + save/resume, time-to-first-value instrumentation — all built and live. Engine live-tested July 22: all 5 trigger types end-to-end against the real Claude API, every session completed all 8 fields, win-type session captured a named person, save/resume proved, export-time PII scrub verified. Deploy verified July 23: commits `f1879bc`+`80c753b` on origin/main, Vercel build succeeded (live /codify bundle contains the trigger-router UI), `p0.5-methodology-entity-guardrails.sql` columns confirmed on live Supabase over REST (pattern_records: trigger_type/method/entity_map/session_start/framework_rendered_at/time_to_first_value_seconds; profiles: persona), /api/codify auth guard returns 401 unauthenticated. **Still open: a real browser click-through of a full /codify session on production.** Full detail: DECISION-LOG 2026-07-22 (Methodology Router + Entity Map).
...
| **P-0.5 Methodology Router + Entity Map** | ✅ **SHIPPED Jul 23.** Deployed to production (`80c753b`), Vercel build verified serving, migration applied + verified on LIVE Supabase. Open: real browser click-through of a full /codify session on prod. | ⭐⭐⭐ Feeds P-1 (persona/org), P-2 (conflict inputs), P-4 (entity signals + auto-pairing), P-4.5 (named mentions) |
